...
function addOneMoreEducation() {
  const eduContainer = document.querySelector(
    ".educationSectionContainer"
  ) as HTMLDivElement;
  const addOneMoreEducationBtn = document.querySelector(
    "#education .addSectionBtn"
...
function addSkill() {
  ...
  const addSkillBtn = document.querySelector(
    "#skills .addSectionBtn"
  ) as HTMLButtonElement;
...
function removeSkill() {
  ...
  skillContainer.addEventListener('click',(e)=>{
    const target = e.target as HTMLElement;
    if(target.classList.contains('fa-circle-xmark')) target.parentElement?.remove()
  })
